...
## 태그 내용을 무시하게 해주는 raw 태그

raw태그는 마크다운 상에서 출력 자체가 되지 않으므로 설명이 나와있는 관련 링크로 대체합니다. 짧게 줄여 말하면 태그 내용을 인식하는 것을 무시하게 해주는 태그입니다. 코드 안에 있는 반복문이나 태그 내용들을 모두 동작하지 않게 만들어줍니다. 다음에 나오는 5번 파일 다운로드에서 실제로 어떻게 쓰이는 지를 보여드립니다.
