---
layout: post
title: markdown 문법
excerpt: "사소하지만 필요한 것"
categories: [markdown]
link:
comments: true
---

마크 다운으로 블로그 정리를 하는 분들과 나 자신을 위해 기능별로 사용하는 문법들을 정리해 보았습니다. 깃허브 블로그를 사용하신다는 전제하에 진행하므로 Github Flavored Markdown(GFM), PHP Markdown Extra 문법 내용까지 모두 수록하였습니다. 이 포스팅에서 몇 개의 것들은 코드는 동일하지만 제 블로그 테마 CSS에 의해 기본 마크 다운 효과와 다르게 보이는 것도 있으니 이 점 착오없으시길 바랍니다. 또한 유용한 HTML태그들도 같이 보여드립니다. \<>태그 안에 감싸져 있는 것들은 모두 html태그 입니다.

참고한 사이트들 : <br />
<https://gist.github.com/ihoneymon/652be052a0727ad59601>
<http://sergeswin.com/1013>
<https://docs.moodle.org/archive/ko/%EB%A7%88%ED%81%AC%EB%8B%A4%EC%9A%B4%EC%9D%98_%EA%B3%A0%EA%B8%89_%ED%99%9C%EC%9A%A9>
<http://3slifehiking.tistory.com/4>

# \<Heading>

# 헤딩1
## 헤딩2
### 헤딩3
#### 헤딩4
##### 헤딩5
###### 헤딩6

<h1>헤딩1</h1>
<h2>헤딩2</h2>
<h3>헤딩3</h3>
<h4>헤딩4</h4>
<h5>헤딩5</h5>
<h6>헤딩6</h6>


~~~md
# 헤딩1
## 헤딩2
### 헤딩3
#### 헤딩4
##### 헤딩5
###### 헤딩6

<h1>헤딩1</h1>
<h2>헤딩2</h2>
<h3>헤딩3</h3>
<h4>헤딩4</h4>
<h5>헤딩5</h5>
<h6>헤딩6</h6>
~~~

### \<Line break>

줄<br />바꿈<br />을<br />해봅니다.

~~~md
줄<br />바꿈<br />을<br />해봅니다.
~~~

# \<Link>

### \<URL link>

<https://hyunseob.github.io/2016/09/25/typescript-introduction/><br />
['일반 링크'](http://han41858.tistory.com/14)<br />
['새창으로 링크'](http://han41858.tistory.com/14){: target="_blank" }<br />
['새창으로 버튼 링크'](http://han41858.tistory.com/14){: .btn.btn-default target="_blank" }<br />

~~~md
<https://hyunseob.github.io/2016/09/25/typescript-introduction/><br />
['일반 링크'](http://han41858.tistory.com/14)<br />
['새창으로 링크'](http://han41858.tistory.com/14){: target="_blank" }<br />
['새창으로 버튼 링크'](http://han41858.tistory.com/14){: .btn.btn-default target="_blank" }<br />
~~~

### \<Inline link>

[Google](http://www.google.co.kr “구글”)

~~~md
//기본 문법 : [링크](http://example.com "링크 제목")
[Google](http://www.google.co.kr “구글”)
~~~

### \<Reference link>

[Google][1]
[Naver][2]
[1]: http://google.com/ “구글”
[2]: http://naver.com/ “네이버”

~~~md
//기본 문법 : [링크1][1] [1]: http://example1.com/ "링크제목1"

[Google][1]
[Naver][2]
[1]: http://google.com/ “구글”
[2]: http://naver.com/ “네이버”
~~~

### \<Internal (Anchored) Links>

[목차](#index)

~~~md
//기본 문법 : [링크](#id) 내부 링크

[목차](#index)
~~~

# \<Image>

### 파일 경로로 이미지 삽입<br />
![Smithsonian Image](/img/2017-07-07-01.jpg)

~~~md
![Smithsonian Image](/img/2017-07-07-01.jpg)
~~~

### 인터넷 주소로 이미지 삽입<br />
![Smithsonian Image](https://t1.daumcdn.net/friends/prod/category/M001_friends_ryan2.jpg)

~~~md
![Smithsonian Image](https://t1.daumcdn.net/friends/prod/category/M001_friends_ryan2.jpg)
~~~

### 링크 이미지 참조

![Smithsonian Image][1]

[1]:/img/2017-07-07-01.jpg

~~~md
![Smithsonian Image][1]

[1]:/img/2017-07-07-01.jpg
~~~

### 이미지에 css 입히기<br />
![Smithsonian Image](/img/2017-07-07-01.jpg){: .center}<br />
![Smithsonian Image](/img/2017-07-07-01.jpg){: .pull-left}<br />
![Smithsonian Image](/img/2017-07-07-01.jpg){: .pull-right}<br />

~~~md
![Smithsonian Image](/img/2017-07-07-01.jpg){: .center}<br />
![Smithsonian Image](/img/2017-07-07-01.jpg){: .pull-left}<br />
![Smithsonian Image](/img/2017-07-07-01.jpg){: .pull-right}<br />
~~~

# \<Emphasis>

*single asterisks*
_single underscores_

**double asterisks**
__double underscores__

***double asterisks***
___double underscores___

<del>underline</del>
~~cancelline~~

~~~md
*single asterisks*
_single underscores_

**double asterisks**
__double underscores__

***double asterisks***
___double underscores___

<del>underline</del>
~~cancelline~~
~~~

# \<Division>

* * *

***

*****

- - -

---------------------------------------

~~~md
* * *

***

*****

- - -

---------------------------------------
~~~

# \<List>
### \<Ordered List>

1. Number1
2. Number2
3. Number3

~~~md
1. Number1
2. Number2
3. Number3
~~~


### \<Unordered List>

* Number1
  * Number1-1
    * Number1-1-1

+ Number1
  + Number1-1
    + Number1-1-1

- Number1
- Number2
- Number3

~~~md
* Number1
  * Number1-1
    * Number1-1-1

+ Number1
  + Number1-1
    + Number1-1-1

- Number1
- Number2
- Number3
~~~

# \<BlockQuote>

> This is a blockquote.

~~~md
> This is a blockquote.
~~~

> This is a blockquote.
>> This is a blockquote.

~~~md
> This is a blockquote.
>> This is a blockquote.
~~~

# \<Remark>

A wonderful serenity has taken possession of my entire soul, like these sweet mornings of spring which I enjoy with my whole heart.[^1] I am alone, and feel the charm of existence in this spot, which was created for the bliss of souls like mine. I am so happy, my dear friend, so absorbed in the exquisite sense of mere tranquil existence, that I neglect my talents. I should be incapable of drawing a single stroke at the present moment; and yet I feel that I never was a greater artist than now.[^2] When, while the lovely valley teems with vapour around me, and the meridian sun strikes the upper surface of the impenetrable foliage of my trees, and but a few stray gleams steal into the inner sanctuary, I throw myself down among the tall grass by the trickling stream; and, as I lie close to the earth, a thousand unknown plants are noticed by me: when I hear the buzz of the little world among the stalks, and grow familiar with the countless indescribable forms of the insects and flies, then I feel the presence of the Almighty, who formed us in his own image, and the breath

[^1]: 1번 주석 내용입니다.
[^2]: [블로그 홈](https://youduk.github.io/)

~~~md
A wonderful serenity has taken possession of my entire soul, like these sweet mornings of spring which I enjoy with my whole heart.[^1] I am alone, and feel the charm of existence in this spot, which was created for the bliss of souls like mine. I am so happy, my dear friend, so absorbed in the exquisite sense of mere tranquil existence, that I neglect my talents. I should be incapable of drawing a single stroke at the present moment; and yet I feel that I never was a greater artist than now.[^2] When, while the lovely valley teems with vapour around me, and the meridian sun strikes the upper surface of the impenetrable foliage of my trees, and but a few stray gleams steal into the inner sanctuary, I throw myself down among the tall grass by the trickling stream; and, as I lie close to the earth, a thousand unknown plants are noticed by me: when I hear the buzz of the little world among the stalks, and grow familiar with the countless indescribable forms of the insects and flies, then I feel the presence of the Almighty, who formed us in his own image, and the breath

[^1]: 1번 주석 내용입니다.
[^2]: [블로그 홈](https://youduk.github.io/)
~~~

# \<table>

테이블은 조금 복잡하니 만약 어려우시면 마크다운 테이블 생성 사이트를 이용하세요.<br />
<http://www.tablesgenerator.com/markdown_tables><br />

테이블 속성1 | 테이블 속성2
---|---
성공|실패
여자|남자
짜장|짬뽕

~~~md
테이블 속성1 | 테이블 속성2
---|---
성공|실패
여자|남자
짜장|짬뽕
~~~

|  <center>가운데 정렬 속성명1</center> |<center>가운데 정렬 속성명2</center>|<center>가운데 정렬 속성명3</center>|
|:--------|:--------|:--------|
|왼쪽 정렬 속성|왼쪽 정렬 속성|왼쪽 정렬 속성|
|왼쪽 정렬 속성|왼쪽 정렬 속성|왼쪽 정렬 속성|
|왼쪽 정렬 속성|왼쪽 정렬 속성|왼쪽 정렬 속성|

~~~md
|  <center>가운데 정렬 속성명1</center> |<center>가운데 정렬 속성명2</center>|<center>가운데 정렬 속성명3</center>|
|:--------|:--------|:--------|
|왼쪽 정렬 속성|왼쪽 정렬 속성|왼쪽 정렬 속성|
|왼쪽 정렬 속성|왼쪽 정렬 속성|왼쪽 정렬 속성|
|왼쪽 정렬 속성|왼쪽 정렬 속성|왼쪽 정렬 속성|
~~~

|  <center>가운데 정렬 속성명1</center> |<center>가운데 정렬 속성명2</center>|<center>가운데 정렬 속성명3</center>|
|:--------:|:--------:|:--------:|
|가운데 정렬 속성|가운데 정렬 속성|가운데 정렬 속성|
|가운데 정렬 속성|가운데 정렬 속성|가운데 정렬 속성|
|가운데 정렬 속성|가운데 정렬 속성|가운데 정렬 속성|

~~~md
|  <center>가운데 정렬 속성명1</center> |<center>가운데 정렬 속성명2</center>|<center>가운데 정렬 속성명3</center>|
|:--------:|:--------:|:--------:|
|가운데 정렬 속성|가운데 정렬 속성|가운데 정렬 속성|
|가운데 정렬 속성|가운데 정렬 속성|가운데 정렬 속성|
|가운데 정렬 속성|가운데 정렬 속성|가운데 정렬 속성|
~~~

# \<Code block>

개발자에게 가장 중요한 기능 중 하나입니다. 지정할 수 있는 언어는 c, c++, c#, clojure, go, java, javascript, python, python3, ruby, scala, swift이 있습니다.(2016년 5월 기준)

코드 블럭을 지정할 수 있는 방법은 여러가지가 있습니다.

기본 마크 다운에서는 코드문자 앞에 4칸 이상 공백이나 탭 하나가 있으면 코드 블럭이 씌워지지만

깃허브에서 쓰는 확장된 마크다운 문법인 Github Flavored Markdown(GFM)에서는 좀 더 추가적인 방법을 제공합니다.


단어 코드 문자열은 `console.log` 이런 식으로 강조 합니다.
역 따옴표를 넣으려면 ``console.log(`)`` 이런 식으로 괄호로 둘러싸서 표현합니다. 합니다.

문자열을 코드로 표현하는 것은 `` `console.log` `` 이런 식으로 표현 합니다.

~~~md
단어 코드 문자열은 `console.log` 이런 식으로 강조 합니다.
역 따옴표를 넣으려면 ``console.log(`)`` 이런 식으로 괄호로 둘러싸서 표현합니다.

문자열을 코드로 표현하는 것은 `` `console.log` `` 이런 식으로 표현 합니다.
~~~



```

~~~
@Service
public class BoardServicelmpl implements BoardService{
	@Inject
	private BoardDAO dao;
}
~~~

```
@Service
public class BoardServicelmpl implements BoardService{
	@Inject
	private BoardDAO dao;
}
```

~~~java
@Service
public class BoardServicelmpl implements BoardService{
	@Inject
	private BoardDAO dao;
}
~~~

```java
@Service
public class BoardServicelmpl implements BoardService{
	@Inject
	private BoardDAO dao;
}
```

~~~java
@Service
public class BoardServicelmpl implements BoardService{
	@Inject
	private BoardDAO dao;
}
~~~

{% highlight java %}
@Service
public class BoardServicelmpl implements BoardService{
	@Inject
	private BoardDAO dao;
}
{% endhighlight %}

# \<Check Box>
- [ ] checkBox1
- [ ] checkBox2
- [ ] checkBox3


## 1. 삽입된 이미지에 CSS 적용

#### 출력

![Smithsonian Image](/img/2017-07-07-01.jpg)
{: .center}

#### 실제 코드

~~~java
//기본 형식
![Smithsonian Image](/img/2017-07-19-1.png)
{: .pull-right}

//[]안에는 이미지 로딩이 실패했을 때 나타나는 이미지 별명을 적습니다.
//()안에는 파일 경로나, 이미지 url을 넣습니다.
//{}안에는 이미지에 줄 css를 넣습니다.
~~~

## 2. 링크 삽입

사실 링크는 그냥 텍스트 안에 주소를 넣으면 알아서 하이퍼링크 처리를 해주기는 하나, 공식적인 방법은 다음과 같습니다.
또한 블로그 내에서 다른 포스트로 링크를 걸어줄 때도 유용할 것 같습니다.

#### 출력


1. [https://rvm.io/rvm/install](https://rvm.io/rvm/install)
2. [http://usabilityetc.com/articles/ruby-on-mac-os-x-with-rvm/](http://usabilityetc.com/articles/ruby-on-mac-os-x-with-rvm/)
3. [http://theeye.pe.kr/archives/1747](http://theeye.pe.kr/archives/1747)
4. [http://macworld.hjsong.net/76](http://macworld.hjsong.net/76)

#### 실제 코드

~~~java

//먼저 처음에 아래와 같은 식으로 사용할 링크를 기재해주어야 합니다.
---
layout: post
title: 나를 괴롭히던 nokogiri 문제 해결 과정 //포스트 제목
excerpt: "nokogiri 에러를 해결하기 위해 알아야 했던 것들에 대해 - 버전관리툴(rvm), 패키지 관리툴(homebrew/apt-get), 숨김 폴더 관리(in finder, terminal)" //포스트 부제
categories: [rails] // 카테고리명

//내가 쓰고자 하는 링크 표시
link: https://rvm.io/rvm/install
link: http://usabilityetc.com/articles/ruby-on-mac-os-x-with-rvm/
link: http://theeye.pe.kr/archives/1747
link: http://macworld.hjsong.net/76

comments: true //disqus 댓글 사용 문구
---

//그리고 아래와 같이 본문에 적으면 위와 같은 효과가 나타납니다.
1. [https://rvm.io/rvm/install](https://rvm.io/rvm/install)
2. [http://usabilityetc.com/articles/ruby-on-mac-os-x-with-rvm/](http://usabilityetc.com/articles/ruby-on-mac-os-x-with-rvm/)
3. [http://theeye.pe.kr/archives/1747](http://theeye.pe.kr/archives/1747)
4. [http://macworld.hjsong.net/76](http://macworld.hjsong.net/76)
~~~

## 3. '\<'를 사용할 때 !!

#### 출력

2.4. Optional\<T>와 OptionalINT

Optional\<T>는 지네릭 클래스로 'T타입의 객체'를 감싸는 래퍼 클래스이다. 그래서 Optional타입의 객체에는 모든 타입의 참조변수를 담을 수 있다.

#### 실제 코드

~~~java
//<는 html태그로 인식을 하기 때문에 꼭 \를 앞에 붙여줘야 합니다. 그렇지 않으면 출력이 제대로 되지 않거나 컴파일 에러가 발생합니다.
2.4. Optional\<T>와 OptionalINT

Optional\<T>는 지네릭 클래스로 'T타입의 객체'를 감싸는 래퍼 클래스이다. 그래서 Optional타입의 객체에는 모든 타입의 참조변수를 담을 수 있다.
~~~


## 4. html 태그를 무시하게 해주는 raw 태그

raw태그는 마크다운 상에서 출력 자체가 되지 않으므로 설명이 나와있는 관련 링크로 대체합니다. 짧게 줄여 말하면 html태그로 인식하는 것을 무시하게 해주는 태그입니다. 다음에 나오는 5번 파일 다운로드에서 실제로 어떻게 쓰이는 지를 보여드립니다.

https://stackoverflow.com/questions/24102498/escaping-double-curly-braces-inside-a-markdown-code-block-in-jekyll


## 5. 파일 다운로드

내 블로그에서 방문자가 파일을 다운로드 하게 하고 싶을 때 링크를 거는 방법입니다.
##### 여기서 raw 태그를 사용하지 않으면 마크다운 컴파일 에러가 발생합니다.

#### 출력

사용한 SQL 소스 파일 [다운로드][1]

{% raw %}
[1]:{{https://youduk.github.io/}}/download/edu_tables_all.sql
{% endraw %}

#### 실제 코드

![Smithsonian Image](/img/2017-07-07-01.png)
