...
그러면 이렇게 폴더가 VS code에 추가된 것을 볼 수 있습니다. 만약 처음 하시는 분들이라면 작업영역이 제목없음이 아니고 해당 폴더 명으로 되어있습니다. 이전에 여러 폴더를 같이 작업한 적이 있으신 분이라면 저처럼 작업 영역이라고 뜨니, 당황하지마세요. 이 때는 먼저 파일 - 작업영역닫기를 눌러서 제목없음으로 되어있는 작업영역을 먼저 종료하고, 해당 프로젝트 폴더를 다시 여시면 작업영역이 프로젝트 폴더로 지정됩니다.
...
### 컴파일 된 css 파일을 원하는 위치에 export 하기

이제 sass폴더 안에  css파일을 컴파일해서 저장하는 것까지는 완료하였습니다. 하지만 보통 프로젝트에서는 컴파일 된 css 파일이 css폴더나 style 폴더에 들어있는 것을 볼 수 있는데요. 일일히 sass 폴더에서 컴파일 된 파일을 옮길 수는 없겠죠? 간단하게 컴파일 될 때마다 저장경로를 바꿀 수 있는 방법이 있습니다.

다시 Live Sass Compiler 확장 프로그램 소개를 열어보세요. 그리고 맨 아래로 스크롤을 내리면 마지막 챕터에 다음과 같은 부분이 있습니다. 이 부분을 복사해주세요.

![](/img/vscode28.png)

그리고 왼쪽 하단에 톱니바퀴를 누르고 설정을 누른 다음, 오른 쪽 탭에 작업영역(WORKSPACE SETTINGS)를 클릭하고 모든 내용을 지운 후에 아래 이미지와 같이 복사 붙여넣기 해주세요.

![](/img/vscode29.png)

눈치 채신 분도 계시겠지만, "liveSassCompile.settings.formats"안의 savaPath 옵션이 컴파일 된 css파일을 저장하는 경로입니다. 이 경로를 바꿔 주면 우리가 원하는 폴더에 컴파일 된 css 파일을 저장할 수 있습니다. 현재 savePath 옵션이 "/css"로 되어 있죠? 이 말은 css폴더에 컴파일 된 css파일을 저장하겠다는 말입니다.

확인을 위해서 먼저 아래와 같이 css라는 이름의 폴더를 만들어볼게요.

![](/img/vscode30.png)

그리고 scss파일을 연 후 하단의 Watch Sass 버튼을 클릭해보면, 원래 sass폴더에 저장되던 css 파일이 새로 만든 css폴더 안에 생성된 것을 볼 수 있습니다.

![](/img/vscode31.png)

그럼 이제는 작업영역의 savePath 경로를 다음과 같이 바꿔볼까요?

![](/img/vscode32.png)

'~' 는 현재 파일을 나타냅니다.

'~/' 는 현재 파일이 있는 폴더를 나타냅니다.

'~/..' 는 현재 파일이 있는 폴더의 한 단계 바깥 위치로 이동하는 것을 나타냅니다.

'~/../style' 는 현재 파일이 있는 폴더에서 한 단계 바깥 위치로 나가서 다시 style이라는 이름의 폴더를 찾는 것을 나타냅니다.

 따라서 '~/../style'은 scss 파일이 있는 곳에서 한단 계 위로 올라갔다가 다시 style이라는 폴더를 찾아서 내려오는 경로입니다. 하지만 지금 style이란 폴더가 없는데요. 이 작업 환경을 저장하고 Watch Sass를 클릭하면 어떻게 될까요? 다음과 같이 style폴더를 새로 만들고 저장하게 됩니다. 

![](/img/vscode33.png)

이제 여러분들도 컴파일된 css 파일을 원하는 위치에 저장할 수 있겠죠?



이번 포스팅에서는 VS code에서 Sass 컴파일 하는 방법을 알아봤습니다. 막히시는 부분이나 궁금하신 부분이 있으면 댓글, 메시지 부탁드려요 :-)